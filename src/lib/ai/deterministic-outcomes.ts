 ...
 * - Analytic closed forms whenever drag is irrelevant:
...
export function dropOutcome(scene: DropScene): DropOutcome | null {
  ...
  if (Math.abs(timeA - timeB) < DROP_TIE_THRESHOLD_S) return "tie";
...
export function projectileOutcome(
  ...
  const difference = flight.range - scene.targetDistance;
  if (Math.abs(difference) <= projectileHitTolerance(scene.targetDistance)) {
    return "hit";
  }
  return difference < 0 ? "undershoot" : "overshoot";
}
