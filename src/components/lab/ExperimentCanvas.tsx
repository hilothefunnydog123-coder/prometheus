...
import { buildEvidence, type SimulationEvidence } from "@/src/lib/physics/evidence";
        ...
        return (
          <group key={object.id}>
            <RigidBody
              ...
              colliders={false}
              restitution={0.08}
              linearDamping={scene.airDensity * object.dragCoefficient * object.radius * 0.26}
              canSleep
            >
...
function World({ spec, launched, capturing, onComplete }: Omit<ExperimentCanvasProps, "runToken">) {
  return (
    <>
...
export function ExperimentCanvas({ spec, runToken, launched, capturing, onComplete }: ExperimentCanvasProps) {
  ...
  return (
    ...
    >
      <World key={`${spec.id}-${runToken}-${launched ? "live" : "ready"}`} spec={spec} launched={launched} capturing={capturing} onComplete={onComplete} />
    </Canvas>
  );
}
