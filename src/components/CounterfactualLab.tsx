...
import type {
  ...
  ExperimentSpec,
  GradeBand,
} from "@/src/lib/contracts/experiment";
import { experimentSpecSchema } from "@/src/lib/contracts/experiment";
import {
  demoExperiments,
  demoForPrompt,
  dropDemo,
} from "@/src/components/lab/demo-experiments";
import { EvidenceChart } from "@/src/components/lab/EvidenceChart";
import {
  applyCounterfactual,
  type SimulationEvidence,
  updateScenePath,
} from "@/src/lib/physics/evidence";

const ExperimentCanvas = dynamic(
  () => import("@/src/components/lab/ExperimentCanvas").then((module) => module.ExperimentCanvas),
  {
    ssr: false,
    loading: () => (
...
const exampleMeta = [
  { icon: CircleGauge, kicker: "FREE FALL", question: "Do heavier objects fall faster?", spec: demoExperiments[0] },
  { icon: Target, kicker: "PROJECTILES", question: "Why does a thrown ball follow an arc?", spec: demoExperiments[1] },
  { icon: Atom, kicker: "OSCILLATION", question: "Does a heavier pendulum swing faster?", spec: demoExperiments[2] },
];
  ...
  return (
    <div className="compiler-overlay" role="status" aria-live="polite">
      <div className="compiler-card">
        ...
        <h2>{compileStages[stage].label}</h2>
        <p>{compileStages[stage].detail}</p>
    ...
    <div className="panel-content prediction-panel">
      ...
      <div className="choice-list">
        {spec.prediction.choices.map((choice, index) => (
          <button key={choice.id} className={selected === choice.id ? "selected" : ""} onClick={() => setSelected(choice.id)}>
      ...
      <button className="run-button" disabled={!selected} onClick={onRun}>
        <Play size={17} fill="currentColor" /> Run experiment
      </button>
    </div>
  );
}
...
function LabWorkspace({
  ...
  const [mastery, setMastery] = useState(28);
  ...
  const onComplete = useCallback(
    (result: SimulationEvidence) => {
      ...
      if (phase === "running") {
        ...
        setMastery(correct ? 52 : 41);
        setPhase("evidence");
      } else if (phase === "counterfactual-running") {
        setTransferCorrect(correct);
        setMastery(correct ? 88 : 67);
        setPhase("complete");
      }
    },
    [chosen?.outcomeKey, phase, setPhase],
  );
    ...
    <main className="lab-workspace">
      <div className="lab-topline">
        <div className="lab-title-block">
          ...
          <div><h1>{spec.title}</h1><p>{spec.objective}</p></div>
        </div>
      ...
      <div className="lab-grid">
        <section className="simulation-stage">
          ...
          <div className="canvas-shell">
            <ExperimentCanvas spec={spec} runToken={runToken} launched={launched} capturing={capturing} onComplete={onComplete} />
        ...
        <aside className="learning-panel">
          {(phase === "predicting" || phase === "counterfactual-predicting" || phase === "running" || phase === "counterfactual-running") && (
            <PredictionPanel spec={spec} selected={selected} setSelected={setSelected} onRun={run} counterfactual={counterfactual} />
          )}
...
export function CounterfactualLab() {
  ...
  const compile = async (directSpec?: ExperimentSpec) => {
    ...
    const compileRequest = async () => {
      ...
      try {
        ...
        // The frontend stays fully demonstrable while the compiler branch is absent or offline.
      }
    };
  ...
  return (
    <div className="app-shell">
      ...
      {phase === "compiling" && <><Landing prompt={prompt} setPrompt={setPrompt} gradeBand={gradeBand} setGradeBand={setGradeBand} image={image} imagePreview={imagePreview} setImage={setImage} setImagePreview={setImagePreview} error={error} compile={compile} /><CompilerOverlay stage={stage} /></>}
      {inLab && <LabWorkspace key={spec.id} spec={spec} setSpec={setSpec} phase={phase} setPhase={setPhase} onExit={reset} />}
    </div>
  );
}
